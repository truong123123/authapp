...
class _OAuth2RedirectScreenState extends State<OAuth2RedirectScreen> {
  ...
  Future<void> _handleRedirect() async {
    ...
    if (widget.token != null && widget.token!.isNotEmpty) {
      ...
      await authProvider.checkAuthStatus(); // Reload user profile using new token
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Đăng nhập Social thất bại.'), backgroundColor: Colors.red),
        );
        Navigator.pushReplacement(
          context,
